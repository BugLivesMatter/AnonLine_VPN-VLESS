...
class VlessVPNApp(QMainWindow):
    ...
    def closeEvent(self, event):
        ...
        if self.is_connected:
            ...
            if reply == QMessageBox.Yes:
                # Восстанавливаем настройки перед закрытием
            ...
            # Просто закрываем если VPN не подключен
            event.accept()

    def start_xray(self, config_path):
        """Запускает Xray с максимальными настройками анонимности"""
        try:
            xray_path = "xray.exe"
            ...
            time.sleep(5)
            self.log("Xray запущен с максимальными настройками анонимности")
    ...
    def check_connection(self):
        ...
        if self.is_connected:
            try:
                ...
                self.connect_btn.setStyleSheet("background-color: #8B0000; color: white;")
    ...
    def connect(self):
        ...
        if not self.save_key():
            self.log("Ошибка: Не удалось сохранить ключ")
            return
        ...
        self.log("Парсинг VLESS-ключа выполнен успешно")
        self.log(f"Сервер: {params['server']}:{params['port']}")
        self.log(f"ID: {params['uuid'][:8]}...")
